      ...
      <div style={{maxWidth:720,margin:"0 auto",padding:"40px 24px"}}>
        ...
        {post ? (
          <>
            ...
            <div style={{display:"flex",gap:8,marginBottom:12,flexWrap:"wrap"}}>{(post.tags||[]).map(t=><span key={t} style={{fontSize:10,background:"rgba(201,168,76,.1)",color:"#c9a84c",padding:"3px 10px",borderRadius:12,fontWeight:600}}>{t}</span>)}</div>
                ...
                {posts.map(p => (
                  <div key={p.id} onClick={() => setSelected(p.slug)} style={{cursor:"pointer",background:"rgba(255,255,255,.03)",border:"1px solid rgba(255,255,255,.06)",borderRadius:18,overflow:"hidden",transition:"all .2s"}}>
                    ...
                    <div style={{padding:"20px 22px"}}>
                      <div style={{display:"flex",gap:6,marginBottom:8}}>{(p.tags||[]).map(t=><span key={t} style={{fontSize:9,background:"rgba(201,168,76,.1)",color:"#c9a84c",padding:"2px 8px",borderRadius:10,fontWeight:600}}>{t}</span>)}</div>
